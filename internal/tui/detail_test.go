...
func TestDetailScreenInit(t *testing.T) {
	...
	sty := newStyles(true)
	keys := defaultKeyMap()
	screen := newDetailScreen(t.Context(), &mockSearcher{}, nil, nil, nil, nil, "acme", "bundle", &sty, &keys)
...
func TestDetailScreenFetchDetail(t *testing.T) {
	...
	screen := newDetailScreen(t.Context(), searcher, nil, nil, nil, nil, "acme", "bundle", &sty, &keys)
