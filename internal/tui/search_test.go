...
func TestSearchScreenInit(t *testing.T) {
	...
	t.Run("with initial query", func(t *testing.T) {
		...
		sty := newStyles(true)
		keys := defaultKeyMap()
		screen := newSearchScreen(context.Background(), &mockSearcher{}, nil, nil, nil, "agent", &sty, &keys)
	...
	t.Run("without initial query", func(t *testing.T) {
		...
		sty := newStyles(true)
		keys := defaultKeyMap()
		screen := newSearchScreen(context.Background(), &mockSearcher{}, nil, nil, nil, "", &sty, &keys)
...
func TestSearchScreenSearchResult(t *testing.T) {
	...
	sty := newStyles(true)
	keys := defaultKeyMap()
	screen := newSearchScreen(context.Background(), &mockSearcher{}, nil, nil, nil, "test", &sty, &keys)
...
func TestSearchScreenKeyHandling(t *testing.T) {
	...
	t.Run("esc with text clears input", func(t *testing.T) {
		...
		sty := newStyles(true)
		keys := defaultKeyMap()
		screen := newSearchScreen(context.Background(), &mockSearcher{}, nil, nil, nil, "some text", &sty, &keys)
		screen.focusArea = searchFocusInput
